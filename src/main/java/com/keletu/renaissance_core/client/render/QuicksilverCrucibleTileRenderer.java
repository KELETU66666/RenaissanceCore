...
public class QuicksilverCrucibleTileRenderer extends TileEntitySpecialRenderer<QuicksilverCrucibleTile> {
    ...
    public void renderFluid(QuicksilverCrucibleTile cr, double x, double y, double z) {
        ...
        if (cr.aspects.getAmount(Aspect.EXCHANGE) > 0) {
            UtilsFX.renderQuadFromIcon(icon, 1.0f, 1.0f - recolor / 3.0f, 1.0f - recolor, 1.0f - recolor / 2.0f, RFBlocks.quicksilver_crucible.getPackedLightmapCoords(cr.getWorld().getBlockState(cr.getPos()), cr.getWorld(), cr.getPos()), 771, 1.0f);
        }
        GL11.glPopMatrix();
    }

    @Override
    public void render(QuicksilverCrucibleTile te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
